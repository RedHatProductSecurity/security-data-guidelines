...
def run_syft(builddir):
    syft = subprocess.run(cwd=os.path.dirname(builddir), stdout=subprocess.PIPE, args=[
        'syft',
        '-o',
        'spdx-json',
    ...
    for relationship in syft_rels:
        if (relationship['spdxElementId'] == 'SPDXRef-DOCUMENT' and
...
def handle_srpm(filename, name):
    with TemporaryDirectory(dir=os.getcwd()) as srcdir:
        subprocess.run(cwd=srcdir, args=[
            'rpm',
            '-D',
            f"%_topdir {srcdir}",
            '-i',
